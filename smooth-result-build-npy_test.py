...
if not os.path.exists('dataResult/scores_AutoML_smooth.npy') or reload:
    ...
    for i, station in enumerate(station_ids):
        ...
        for k, eof in enumerate(range(1, 7)):
            ...
            r2 = r2_score(reals, preds)
            scores_ML[i, k] = r2
    np.save('dataResult/scores_AutoML_smooth', scores_ML)
else:
    scores_ML = np.load('dataResult/scores_AutoML_smooth.npy')
   
path = '/p/lustre2/shiduan/AUTOLR-predictions-smooth/'
if not os.path.exists('dataResult/scores_AutoLR_smooth.npy') or reload:
    scores_ML = np.zeros((25, 6))
    for i, station in enumerate(station_ids):
        lag = 0
        for k, eof in enumerate(range(1, 7)):
            ...
            r2 = r2_score(reals, preds)
            scores_ML[i, k] = r2
    np.save('dataResult/scores_AutoLR_smooth', scores_ML)
else:
...
path = '/p/lustre2/shiduan/AUTOML-predictions-smooth/'
if not os.path.exists('dataResult/scores_AutoML_smooth_lag3.npy') or reload:
    scores_ML = np.zeros((25, 6))
    for i, station in enumerate(station_ids):
        lag = 0
        for k, eof in enumerate(range(1, 7)):
            ...
            r2 = r2_score(reals, preds)
            scores_ML[i, k] = r2
    np.save('dataResult/scores_AutoML_smooth_lag3', scores_ML)
else:
    scores_ML = np.load('dataResult/scores_AutoML_smooth_lag3.npy')


path = '/p/lustre2/shiduan/AUTOLR-predictions-smooth/'
if not os.path.exists('dataResult/scores_AutoLR_smooth_lag3.npy') or reload:
    scores_ML = np.zeros((25, 6))
    for i, station in enumerate(station_ids):
        lag = 0
        for k, eof in enumerate(range(1, 7)):
            ...
            r2 = r2_score(reals, preds)
            scores_ML[i, k] = r2
    np.save('dataResult/scores_AutoLR_smooth_lag3', scores_ML)
else:
    scores_ML = np.load('dataResult/scores_AutoLR_smooth_lag3.npy')
