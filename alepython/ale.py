...
def _ax_title(ax, title, subtitle=None):
    """Add title to axis.
...
def _ax_quantiles(ax, quantiles, twin="x"):
    ...
    getattr(ax_mod, "set_{twin}ticks".format(twin=twin))(label_quantiles)
    # Set the corresponding tick labels.

    # Calculate tick label percentage values for each quantile (bin edge).
    percentages = (
        100 * np.arange(len(label_quantiles), dtype=np.float64) / (len(label_quantiles) - 1)
    )
...
def _second_order_quant_plot(
    ...
    **kwargs : contourf properties, optional
    ...
    values = ale_interp(x, y)
    vmin = np.min(values)
    vmax = np.max(values)
    vmax = np.max([np.abs(vmin), vmax])
    norm = colors.TwoSlopeNorm(vmin=-vmax*.8, vcenter=0, vmax=vmax*.8)
    CF = ax.contourf(X, Y, values, cmap="bwr", levels=30, alpha=0.7, norm=norm, **kwargs)
    ...
    # fig.colorbar(cm.ScalarMappable(norm=norm, cmap="bwr"), cax=cbar_ax)
    # plt.tight_layout()
    return norm
...
def _second_order_ale_quant(predictor, train_set, features, bins, first=False):
    ...
    first: bool
        Whether to keep the first-order effects. Default: False
    Returns
    -------
            ...
            if monte_carlo:
                ...
                for k, rep in enumerate(mc_replicates):
                    ...
                    if features_classes is None:
                        ...
                        # print('MC quantiles:', mc_quantiles[0], mc_quantiles[-1])
