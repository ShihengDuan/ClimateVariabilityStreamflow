# feature importance by using only 1 input variable. 
# ```include```
import numpy as np
...
def get_prediction(model_type, station, mod, lag3, seed):
    if lag3:
        pred_path = '/p/lustre2/shiduan/'+model_type.upper()+'-predictions-smooth/include-'+mod+'/'+station+'/'+station+'-EOF-6-seed-'+str(seed)+'-pred_lag3.npy'
        real_path = '/p/lustre2/shiduan/'+model_type.upper()+'-predictions-smooth/include-'+mod+'/'+station+'/'+station+'-EOF-6-seed-'+str(seed)+'-real_lag3.npy'
    else:
        pred_path = '/p/lustre2/shiduan/'+model_type.upper()+'-predictions-smooth/include-'+mod+'/'+station+'/'+station+'-EOF-6-seed-'+str(seed)+'-pred.npy'
        real_path = '/p/lustre2/shiduan/'+model_type.upper()+'-predictions-smooth/include-'+mod+'/'+station+'/'+station+'-EOF-6-seed-'+str(seed)+'-real.npy'
...
if re_load:
    for ind, station in enumerate(station_ids):
        ...
        print(Lasso_output)
        print()
    r2s_include = [r2s_linear, r2s_lasso, r2s_ridge, r2s_lod, r2s_ml]
    with open('r2s_include_eof_smooth.p', 'wb') as pfile:
        pickle.dump(r2s_include, pfile)
else:
...
fig.legend(handles=[PNA5_patch, PDO5_patch, PDO3_patch], fontsize=12, 
           loc='upper center', ncol=3, bbox_to_anchor=(0.5, .9))
plt.tight_layout()
plt.savefig('Cal-firstVariable-spatial-include.png', dpi=150, bbox_inches='tight')
...
for i in range(5):
    ...
    # ax.set_ylim([0, 0.4])
plt.tight_layout()
plt.savefig('Cal-median-result-include.png', dpi=150, bbox_inches='tight')
print('Done')
